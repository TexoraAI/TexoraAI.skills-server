...
public class CodingProblemController {
    ...
    // GET /api/v1/problems/usage/create — read-only preview, not feature-gated
    @GetMapping("/usage/create")
    public ResponseEntity<Map<String, Object>> getCreateUsage(HttpServletRequest httpRequest) {
        return ResponseEntity.ok(assessmentUsageService.getUsageStatus(
                AssessmentUsageLimits.Action.CODING_CREATE, callerEmail(), orgId(httpRequest)));
    }
