...
public class QuizController {
    ...
    // TRAINER: USAGE PREVIEW — read-only, not feature-gated (same ungated style as GET /{id})
    @GetMapping("/usage/create")
    public ResponseEntity<Map<String, Object>> getCreateUsage(Authentication auth, HttpServletRequest request) {
        return ResponseEntity.ok(assessmentUsageService.getUsageStatus(
                AssessmentUsageLimits.Action.QUIZ_CREATE, auth.getName(), orgId(request)));
    }
