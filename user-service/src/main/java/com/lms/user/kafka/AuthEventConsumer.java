...
//    }
//
//    // WHY: Keeps user-service in sync when auth-service deletes a user
//    @Transactional
...
//        try {
//            // payload JSON was stuffed into organizationId field by AuthService.updateProfile()
...
//                    .orElseGet(() -> { TrainerProfile np = new TrainerProfile(); np.setUser(user); return np; });
...
//                    String plat = (String) data.get("platforms");
//                    p.setPlatforms(java.util.Arrays.asList(plat.split(",")));
//                }
//                trainerProfileRepo.save(p);
//
//            } else if ("student".equalsIgnoreCase(role)) {
//                StudentProfile p = studentProfileRepo.findByUser_Email(event.getEmail())
//                    .orElseGet(() -> { StudentProfile np = new StudentProfile(); np.setUser(user); return np; });
...
//            }
//            // "Manager" role intentionally NOT handled here — that data
//            // correctly belongs in Organization (auth-service), and is already
//            // written directly via authService.updateAdminOrgProfile, once
//            // ProfileDetailsForm.jsx is fixed (see message below) to call that
//            // endpoint instead of /auth/me/profile for Manager role.
//
