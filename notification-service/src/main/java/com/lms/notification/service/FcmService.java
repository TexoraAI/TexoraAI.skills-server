...
public class FcmService {
    ...
    public void sendToUser(String userEmail, String title, String body, String type) {
        ...
        // ✅ FIX: case-insensitive — student@gmail.com == Student@gmail.com
        List<DeviceToken> tokens = deviceTokenRepo.findByUserEmailCaseInsensitive(userEmail);
        ...
        System.out.println("📱 FCM sending to " + tokens.size() + " device(s) for: " + userEmail);
    ...
    public void sendToTokenList(List<String> fcmTokens, String title,
        ...
        System.out.println("📢 FCM sending to " + fcmTokens.size() + " role token(s)");
    ...
    private void sendToToken(String token, String title, String body,
        ...
        try {
            ...
            String response = FirebaseMessaging.getInstance().send(message);
            System.out.println("✅ FCM sent role=" + userRole + " → " + response);

        } catch (FirebaseMessagingException e) {
            System.out.println("❌ FCM failed: " + token + " → " + e.getMessage());
            if (e.getMessagingErrorCode() == MessagingErrorCode.UNREGISTERED
                    || e.getMessagingErrorCode() == MessagingErrorCode.INVALID_ARGUMENT) {
                deviceTokenRepo.deleteByFcmToken(token);
                System.out.println("🗑️ Removed stale token");
