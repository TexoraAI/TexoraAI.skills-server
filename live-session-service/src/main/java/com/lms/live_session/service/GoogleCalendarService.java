...
//            String organizationId, String returnTo) {
//try {
//GoogleAuthorizationCodeFlow flow = createFlow();
//return flow.newAuthorizationUrl()
//.setRedirectUri(googleCalendarProperties.getRedirectUri())
...
//    }
     ...
     * fire this twice per hour and double-sync every calendar.
     *
     * Deliberate scope decision: this path bypasses the manual calendar-resync
     * quota (LiveSessionUsageService.checkAndIncrementCalendarResync) entirely —
     * it's not a user-initiated action and can't be blocked mid-job, same
     * exemption as LIVE_AUTO recordings in Feature 1.
     */
    public void scheduledSyncAll() {
        List<CalendarSync> syncs = calendarSyncRepository.findBySyncStatus(SyncStatus.CONNECTED);
        for (CalendarSync sync : syncs) {
