...
public class CalendarSyncScheduler {
    ...
    @Scheduled(fixedRate = 3600000) // every 1 hour
    // Deliberate scope decision: this background sync bypasses the manual
    // resync quota (LiveSessionUsageService.checkAndIncrementCalendarResync)
    // entirely — it's not a user-initiated action and can't be blocked
    // mid-job, same exemption as LIVE_AUTO recordings in Feature 1.
    public void syncAllConnectedCalendars() {
        try {
            googleCalendarService.scheduledSyncAll();
        } catch (Exception e) {
