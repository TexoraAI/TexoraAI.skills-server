...
public class LiveSessionService {
    ...
    public LiveSession createSession(LiveSession session) {
        ...
        // t1 auto-trigger: "Live session scheduled". Never allowed to break
        // session creation if workflow lookup/execution fails.
        try {
 ...
 public LiveSession startSession(Long id) {
     ...
     if (rows == 0) {
         // Another concurrent call already flipped this session to LIVE.
         // It — and only it — is responsible for egress/notifications
         // (and, per below, for firing the t2 trigger).
         System.out.println("[startSession] BLOCKED duplicate start attempt for session " + id
             + " thread=" + Thread.currentThread().getName());
         return repository.findById(id)
             .orElseThrow(() -> new RuntimeException("Session not found: " + id));
     ...
     // t2 auto-trigger: "Live session started". Only fired on the winning
     // path above (rows > 0) so it never double-fires for the same start.
     try {
         aiWorkflowExecutionService.fireTrigger(
    ...
    public LiveSession endSession(Long id) {
        ...
        // t3 auto-trigger: "Live session ended". Never allowed to break
        // session end if workflow lookup/execution fails.
        try {
            aiWorkflowExecutionService.fireTrigger(
