...
public class BatchAssignmentConsumer {
    ...
    @Transactional
    @KafkaListener(topics = "batch-assignment", groupId = "course-service-group")
    public void consume(Map<String, Object> event) {
        ...
        // NEW — read organizationId; will be null for non-org users, that is fine
        String organizationId = event.get("organizationId") != null
                ? event.get("organizationId").toString()
                : null;
        ...
        switch (type) {
            ...
            case "TRAINER_ASSIGNED" -> {
                if (!trainerRepo.existsByTrainerEmailAndBatchId(email, batchId)) {
                    // NEW — pass organizationId into constructor (null-safe for non-org)
